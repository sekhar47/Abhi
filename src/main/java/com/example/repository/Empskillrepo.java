...
//	    filter
   ...
   Optional<EmployeeSkill> findBySkills(Skills skill);
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM EmployeeSkill es WHERE es.user.empid = :empid AND es.skills.skillid = :skillid")
    void deleteByEmpidAndSkillid(String empid, Integer skillid);

    List<EmployeeSkill> findByReviewedFalse();
}
